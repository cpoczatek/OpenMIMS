...
/**
 * Customized implementation of a particle analyzer, which takes an recursive approach to find particles within particles.
 * Only 4-connected wand mode gives consistent results! Thus, diagonally connected ROIs of the same class are still not joined.
 *
...
//    private ImagePlus vis; // used to visualize the found particles (only for debugging)
    ...
    public NrimsParticleAnalyzer(ImagePlus imp, double foreground, double background, int minSize){
        ...
        for(int i=0; i<initialMask.length; i++) initialMask[i] = (byte)maskColor;
...
//        vis.close();
    ...
    /**
     * Recursivly finds and traces particles within the specified roi. Recursion alternates between
     * finding a particle (findHoles == false) and finding holes within that particle (findHoles == true).
     * Holes are finally subtracted from the surrounding roi.
...
//                  System.out.println("found " + foundParticle + "\tat depth " + depth + "   in " + boundingRoi); // debug info
                    ...
                    if(findHoles)   ip.setValue(foreground);
                    else            ip.setValue(background);
...
//                    vis.setProcessor("", ip.duplicate()); // just for visualization
        ...
        if(findHoles) foundParticles.add(boundingRoi);
