...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
	...
	private Object tryOpen(String directory, String name, String path) {
		...
		// NRIMS: read .im or .nrrd files from a Cameca nanoSIMS
		/*
		if (name.endsWith(".im") || name.endsWith(".nrrd") || name.endsWith(".nhdr")) {
			System.out.println("HandleExtra, inside if .im/.nrrd");
			System.out.println("path="+path);
			String[] files = {path};
			if(com.nrims.UI.ui==null) {
				System.out.println("call ui.main");
				com.nrims.UI.main(files);
			} else {
				System.out.println("call openinBG");
				com.nrims.UI.ui.openFileInBackground(new File(path));
			}
			//com.nrims.NRIMS_Plugin p = new com.nrims.NRIMS_plugin();
			//p.run(path);
			//fake sucessful opening???
			width = IMAGE_OPENED;
			return null;
		}
		*/
